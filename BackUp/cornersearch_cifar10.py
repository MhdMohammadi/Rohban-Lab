...
trainloader = torch.utils.data.DataLoader(trainset, batch_size=1025,
        ...
        for i in range(n_corners):
            for j in range(dims[1]):
                for k in range(dims[2]):
                    ...
                    perturbed[:, j, k] = torch.clamp(orig_x[:, j, k] * (1  + (2*i - 1) * sigma[:, j, k] * kappa), 0.0, 1.0)
...
def npixels_perturbation(orig_x, dist, sigma):
    ...
    with torch.no_grad():
        ...
        for i in range(orig_x.shape[0]):
            batch_x[i, p11[i], p12[i]] = torch.clamp(batch_x[i, p11[i],p12[i]] * (1 - kappa*sigma[i, p11[i], p12[i]]*(1-d1[i]) + kappa*sigma[i, p11[i],p12[i]]*d1[i]), 0.0, 1.0)
